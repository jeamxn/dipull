...
const Search = ({
  ...
  const putJasupAdminData = async (id: string) => {
    setLoading(true);
    const loading = alert.loading("자습 도우미 학생으로 지정 중 입니다.");
    try{
      const res = await instance.put("/api/teacher/jasup", {
        id
      });
      await getData();
        ...
        <table className="w-full overflow-auto">
          <tbody className="w-full border-y border-text/10 overflow-auto">
            <tr className="w-full">
              ...
              <td className="text-center px-4">추가</td>
            </tr>
            {
              userList.length ? userList.map((v, i) => {
                const click = () => {
                  setSelectedUser(v);
                  putJasupAdminData(v.id);
                };
                return (
                  <tr className="w-full border-y border-text/10" key={i} onClick={() => {}}>
                    ...
                    <td className="text-center px-4 select-none" onClick={() => click()}>
