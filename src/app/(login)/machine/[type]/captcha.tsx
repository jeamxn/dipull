  ...
  return (
    ...
    >
      <div className="bg-background px-4 py-5 rounded shadow-xl flex flex-col items-center justify-center gap-2">
        <p className="text-primary font-bold text-lg">자동 입력 방지 { left === -1 ? "" : `(${left}초 남음)` }</p>
        <img
          src={captcha.image}
          alt="captcha image loading..."
          className="w-80 h-[97px] border border-text/10 rounded bg-white"
        />
        <div className="flex flex-row items-center justify-center gap-1 w-full">
          <input 
            ...
            className="w-full px-4 py-2 h-11 bg-white border border-text/10 rounded"
          />
          <button onClick={getCaptcha}>
            <svg className="w-12 h-11 p-3 cursor-pointer flex flex-row items-center justify-center bg-white border border-text/10 rounded" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
