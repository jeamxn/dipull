          ...
          <section className="flex flex-wrap gap-6 flex-row items-center justify-center">
            {
              loading && gallery.length < 0 ? <p>로딩 중...</p> : gallery.map((v, i) => (
                <a key={i} href={`https://www.instagram.com/p/${v.id}`} target="_blank" rel="noreferrer">
                  <section className="max-w-80 flex flex-col pt-10 pb-8 pr-5 pl-5 gap-3 bg-[#fff] bg-opacity-90 border border-text/10">
                    <Image
                      src={v.thumbnail}
                      alt="gallery"
                      width={2048}
                    ...
                    />
                    <div>
                      <p className="text-[#000] font-normal whitespace-nowrap truncate">{v.text.split("\n")[0]}</p>
                      <p className="text-[#000] font-normal whitespace-nowrap truncate">{v.text.split("\n")[1] || "ㅤ"}</p>
                    </div>
