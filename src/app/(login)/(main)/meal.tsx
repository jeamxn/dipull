...
import { Meal as MealType } from "@/app/api/meal/[date]/get";
import { Day } from "@/utils/date";
import instance from "@/utils/instance";

type MealKey = keyof MealType;
...
const Meal = ({
  ...
  const [meal, setMeal] = React.useState<MealType>(init);
  const [isFirst, setIsFirst] = React.useState(true);
  const today = moment().format("YYYY-MM-DD");

  const getMeal = async () => {
    const formatted = date.format("YYYY-MM-DD");
    if(formatted === today) return setMeal(init);
    setLoading(true);
    try {
      const { data } = await instance.get(`/api/meal/${formatted}`);
      setMeal(data.data);
    }
    catch(e){
      console.error(e);
  ...
  return (
    <article className="flex flex-col gap-3">
        ...
        <article className="flex flex-row gap-0">
          {
            [
              ...
              <button key={i} className="flex flex-row items-center justify-center px-4" onClick={v.onClick}>
                <figure className="h-10 flex flex-row items-center justify-center w-full rounded-md ">
                  {v.icon}
                </figure>
              </button>
            ))
      ...
      <article className="overflow-x-auto overflow-y-hidden snap-x snap-mandatory block">
