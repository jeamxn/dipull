  ...
  return (
    <div className="px-6 w-full relative">
      ...
      <div className="absolute top-0 left-0 px-6 w-full h-full">
        <div className="bg-text/50 p-4 w-full h-full rounded-2xl flex flex-col justify-end gap-1 max-sm:gap-0 max-md:gap-1 max-lg:gap-0">
          <div className="flex flex-row gap-1 justify-start">
            <p className="select-none text-white text-3xl max-sm:text-xl max-md:text-3xl max-lg:text-xl font-bold drop-shadow-2xl">{rank.toLocaleString()}위: {vote.toLocaleString()}표</p>
            <button className="-m-4 p-4">
              <svg className="aspect-square w-8 max-sm:w-5 max-md:w-8 max-lg:w-5" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                <mask id="mask0_943_61" maskUnits="userSpaceOnUse" x="0" y="0" width="25" height="25">
                  <rect className="fill-white" x="0.866638" y="0.229004" width="24" height="24"/>
                </mask>
                <g mask="url(#mask0_943_61)">
          ...
          <p className=" select-none text-white text-xl max-sm:text-base max-md:text-xl max-lg:text-base font-bold drop-shadow-2xl">{title}</p>
        </div>
      </div>
    </div>
  );
