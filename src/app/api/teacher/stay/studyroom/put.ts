...
const PUT = async (
  ...
  const stayCollection = client.db().collection("studyroom");
  await stayCollection.deleteMany({});
  const insert = await stayCollection.insertMany(data);

  if(!insert.insertedCount) return new NextResponse(JSON.stringify({
    success: false,
    message: "저장에 실패했습니다.",
  }), {
    status: 500,
